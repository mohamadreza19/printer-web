...
import { FormateDateWithExpire } from "../../../../../utility/useFormetDate";
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.data && "pages" in this.props.data) {
      ...
      if (prevState.page !== newPageCount) {
        ...
        for (let page of this.props.data.pages) {
          mapedData = mapedData.concat(this.readCompanies(page.items));
        }
...
const CompanyCard = ({ item, language }) => {
  return (
    <section
      style={{
        minHeight: "62px",
    ...
    >
      <span className="me-2">{item.companyName || "null"}</span>
      <span className="mx-3">
        {t("createBy")} : {item.username || "null"}{" "}
      </span>
      <span>
        {t("lastPrint")} :‌
        {item.lastPrint
          ? FormateDateWithExpire(item.lastPrint, language, false).date
          : "null"}
      </span>
      <span>
        {t("print")} :{item.countOfPrintedProducts || "null"}
      </span>
      <span>
        <svg
          width="23"
